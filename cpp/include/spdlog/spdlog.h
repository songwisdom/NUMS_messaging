...
// ==== BEGIN: global enum→underlying integer formatter ====
#ifndef SMSC_GLOBAL_ENUM_FMT_DEFINED
#define SMSC_GLOBAL_ENUM_FMT_DEFINED
...
// 모든 enum에 대해 기본적으로 언더라이잉 정수로 포맷
template <typename T>
struct formatter<T, char, std::enable_if_t<std::is_enum_v<T>>>
    : formatter<std::underlying_type_t<T>> {

  using base = formatter<std::underlying_type_t<T>>;
  ...
  auto format(T value, FormatContext& ctx) const {
    ...
    return base::format(static_cast<U>(value), ctx);
  }
};

} // namespace fmt

#endif // SMSC_GLOBAL_ENUM_FMT_DEFINED
// ==== END: global enum→underlying integer formatter ====

// ==== BEGIN: fmt adapters (ranges/ostream/chrono/std) ====
#if defined(SPDLOG_FMT_EXTERNAL)
