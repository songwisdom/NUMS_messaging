/***********************************************************
*
* $Header: /home/cvs/smsc/skt-smsc/include/ksig7/TCAP_ext.h,v 1.1 2008/07/21 05:12:47 prowoni Exp $
* $Source: /home/cvs/smsc/skt-smsc/include/ksig7/TCAP_ext.h,v $
* $Revision: 1.1 $
* $Locker:  $
* 
***********************************************************/
...
typedef enum tcx_transport_return_reason_enum {
  ...
  TCX_SKT_INVALID_MULTI_PC = 200,  /* Notify SC Routing in Multi OPC for SKT */
...
#ifndef TCAP_ANSI90
 tc_dialog_portion        dialog_portion;       /* Application context and 
                                                   user info                */
#endif /* TCAP_ANSI90 */
...
typedef enum tcx_tcap_service_enum {TCX_TCAP_STD=0} tcx_tcap_service;
...
/* Backward compatibility, should not be used in new code
   since it may be removed in further release
*/

#define TC_control(connection_id, context, command, parameter) \
        TCX_control(connection_id, context, command, parameter)

#define TC_select_mask(maxFd, read_mask, write_mask, excep_mask, timeout) \
        TCX_select_mask(maxFd, read_mask, write_mask, excep_mask, timeout)

#define TC_cnx_handler(num_fd, read_mask, write_mask, excep_mask, num_cnx, cnx_ids) \
        TCX_cnx_handler(num_fd, read_mask, write_mask, excep_mask, num_cnx, cnx_ids)
...
/* By default, the error codes of the TCX_xxx calls are the same as
for their TC_xxx counterpart.  Otherwise a comment specifies what are
the differences.

Note that, due to ANSI C type checking, it is not possible to use old
TC_... structures instead of the new TCX_... ones in the new calls,
and vice-versa. Therefore, the only type of error which may occur in
an application using old and new calls is a mismatch of connection
ids, that is using an old TC_... call with a connection id returned by
TCX_open, or the converse.  In these cases, the call fails, with
tc-errno set to TCE_ILLEGAL_CALL.
...
/* The semantics/return value are the same as for TC_send. */
