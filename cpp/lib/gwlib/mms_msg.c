...
static int encode_msgheaders(Octstr *os, List *hdrs)
{
    ...
    version = http_header_value(hdrs, octstr_imm("X-Mms-MMS-Version"));
    transid = http_header_value(hdrs, octstr_imm("X-Mms-Transaction-Id"));
    msgtype = http_header_value(hdrs, octstr_imm("X-Mms-Message-Type"));
    ctype = http_header_value(hdrs, octstr_imm("Content-Type"));
...
MmsMsg *mms_frombinary_ex(Octstr *msg, Octstr *from, char *unified_prefix, List *strip_prefixes)
{
    ...
    if (s && octstr_search(s, octstr_imm("application/vnd.wap.multipart"), 0) == 0)
        _m.ismultipart = 1;
...
loop:
    ...
    if (!hasmsgid) {
        ...
        if (v) {
            http_header_add(mh, "Message-ID", octstr_get_cstr(v));
            octstr_destroy(v);
        }	  
    }
