     ...
     void (*fatal_func) ();
{
  ...
  dbf->file_locking = FALSE;	/* Default to doing file locking. */
  ...
  if (file_stat.st_size == 0)
      ...
      if (dbf->bucket == NULL)
	{
      ...
      dbf->bucket->bucket_avail[0].av_adr = ( 3 * (size_t)dbf->header->block_size);
      ...
      if (num_bytes != dbf->header->block_size)
	{
      ...
      num_bytes = (int)write (dbf->desc, dbf->dir, dbf->header->dir_size);
      if (num_bytes != dbf->header->dir_size)
	{
      ...
      num_bytes = (int)write (dbf->desc, dbf->bucket, dbf->header->bucket_size);
  ...
  else
    {
	 ...
	 header and initialize the hash directory. */
      ...
      num_bytes = (int)read (dbf->desc, &partial_header, sizeof (gdbm_file_header));
      ...
      if (dbf->header == NULL)
	{
      ...
      num_bytes = (int)read (dbf->desc, &dbf->header->avail.av_table[1],
			dbf->header->block_size-sizeof (gdbm_file_header));
      ...
      if (file_pos != dbf->header->dir)
	{
      ...
      num_bytes = (int)read (dbf->desc, dbf->dir, dbf->header->dir_size);
      if (num_bytes != dbf->header->dir_size)
	{
	  gdbm_close (dbf);
	  gdbm_errno = GDBM_FILE_READ_ERROR;
