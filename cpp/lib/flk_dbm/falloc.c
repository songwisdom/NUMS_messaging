     ...
     gdbm_file_info *dbf;
{
  ...
  new_el.av_size = (int)( ( (dbf->header->avail.size * sizeof (avail_elem)) >> 1)
			+ sizeof (avail_block));
  ...
  num_bytes = (int)read (dbf->desc, new_blk, new_el.av_size);
     ...
     gdbm_file_info *dbf;
{
  ...
  av_size = (int)(( (dbf->header->avail.size * sizeof (avail_elem)) >> 1)
            + sizeof (avail_block));
  ...
  num_bytes = (int)write (dbf->desc, temp, av_size);
