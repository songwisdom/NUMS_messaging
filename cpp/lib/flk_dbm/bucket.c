     ...
     int dir_index;
{
  ...
  if (dbf->cache_entry->ca_adr != bucket_adr)
	...
	_gdbm_fatal (dbf, "lseek error");
      ...
      num_bytes = (int)read (dbf->desc, dbf->bucket, dbf->header->bucket_size);
     ...
     int next_insert;
{
  ...
  while (dbf->bucket->count == dbf->header->bucket_elems)
      ...
      for (; index < dbf->bucket->av_count; index++)
	{
      ...
      for (index = (int)dir_start0; index < dir_start1; index++)
	dbf->dir[index] = adr_0;
      for (index = (int)dir_start1; index < dir_end; index++)
	dbf->dir[index] = adr_1;
     ...
     cache_elem *ca_entry;
{
  ...
  if (file_pos != ca_entry->ca_adr)
    _gdbm_fatal (dbf, "lseek error");
  num_bytes = (int)write (dbf->desc, ca_entry->ca_bucket, dbf->header->bucket_size);
  if (num_bytes != dbf->header->bucket_size)
