...
static int
flk_uhc_2_mbtowc (flk_conv_t conv, flk_ucs4_t *pwc, const unsigned char *s, int n)
{
  ...
  if ((c1 >= 0xa1 && c1 <= 0xc6)) 
  {
    if (n >= 2) 
	{
      ...
      if ((c2 >= 0x41 && c2 < 0x5b) || (c2 >= 0x61 && c2 < 0x7b) || (c2 >= 0x81 && c2 < 0xa1)) 
	  {
        ...
        if (i < 3126) {
          *pwc = (flk_ucs4_t) (uhc_2_2uni_main_pagea1[2*row+(col>=42?1:0)] + uhc_2_2uni_pagea1[i]);
          return 2;
        }
      }
...
static int
flk_uhc_2_wctomb (flk_conv_t conv, unsigned char *r, flk_ucs4_t wc, int n)
{
  if (n >= 2) {
