...
static int
flk_uhc_1_mbtowc (flk_conv_t conv, flk_ucs4_t *pwc, const unsigned char *s, int n)
{
  ...
  if ((c1 >= 0x81 && c1 <= 0xa0)) 
  {
    if (n >= 2) 
	{
      ...
      if ((c2 >= 0x41 && c2 < 0x5b) || (c2 >= 0x61 && c2 < 0x7b) || (c2 >= 0x81 && c2 < 0xff)) 
	  {
        ...
        if (i < 5696) 
		{
          *pwc = (flk_ucs4_t) (uhc_1_2uni_main_page81[2*row+(col>=89?1:0)] + uhc_1_2uni_page81[i]);
          return 2;
        }
      }
...
static int
flk_uhc_1_wctomb (flk_conv_t conv, unsigned char *r, flk_ucs4_t wc, int n)
{
  if (n >= 2) {
